...
P getCircleCenter(P A, P B, P C) {
    ...
    ld Ux = ((A.x * A.x + A.y * A.y) * (B.y - C.y) + (B.x * B.x + B.y * B.y) * (C.y - A.y) + (C.x * C.x + C.y * C.y) * (
                 A.y - B.y)) / D;
    ld Uy = ((A.x * A.x + A.y * A.y) * (C.x - B.x) + (B.x * B.x + B.y * B.y) * (A.x - C.x) + (C.x * C.x + C.y * C.y) * (
                 B.x - A.x)) / D;
    return {Ux, Uy};
}
...
vector<P> circle_circle_intersection(P a, ld r, P b, ld R) {
